...
public class TeamOverviewGUI extends javax.swing.JFrame {
    ...
    public TeamOverviewGUI(TeamService teamService, AuthService authService) {
        this.teamService = teamService;
        this.authService = authService;

        readOnly = !authService.getLoggedInMember().hasPermission(Permission.READ_TEAMS);
    ...
    private void initComponents() {
        ...
        setTitle(Application.getTitle("Teams"));
    ...
    private boolean isFilterable(Team team, String filter) {
        return Arrays.stream(filter.split(" ")).anyMatch(param -> team.getId().toString().toLowerCase().contains(param)
            || team.getName().toLowerCase().contains(param)
            || team.getActivity().toLowerCase().contains(param));
    }
    ...
    public final void updateData(boolean fetch) {
        ...
        teams.stream()
            .filter(team -> isFilterable(team, tfSearch.getText().toLowerCase().trim()))
            .map(team -> new Object[] { team.getId().toString(), team.getName(), team.getActivity() })
